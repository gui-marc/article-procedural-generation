...
<article class="container">
  ...
  <section>
    ...
    <p>
      ...
      estão classificados e categorizados, sendo alguns dos principais: “Wave Function Collapse”,
      “Cellular Automata” e “Perlin Noise”.
    </p>
    <p>
  ...
  <section>
    ...
    <p>
      Originalmente lançado em 1980, o jogo “Rogue” é conhecido por ser uma das primeiras aventuras
      gráficas e também por ser precursor de um estilo de jogos, o “Roguelike”, no qual o mapa é
      gerado proceduralmente e, ao morrer, o jogador tem que recomeçar em outro mapa, perdendo todo
      o progresso anterior. Este jogo teve uma grande influência no design de jogos em geral,
      principalmente quando se trata de geração procedural de mundos, uma vez que foi um dos
      primeiros jogos a adotar esta abordagem.
    </p>
    <p>
      O jogo possui uma dinâmica e desafiante forma de se jogar. A cada tentativa, ou run, o jogador
      navega por uma masmorra composta de salas, corredores e monstros, em busca de um artefato
      chamado “Amuleto de Yendor” (Rodney escrito ao contrário). Ao avançar pelos níveis, os
      monstros ficam progressivamente mais difíceis e não é possível voltar até encontrar o amuleto.
      Depois de encontrá-lo, é preciso voltar ao nível inicial e assim, o jogo acaba. Mesmo com
      gráficos primitivos baseados em texto ASCII, o jogo conseguiu desafiar o jogador por fazer com
      que ele tivesse que se adaptar a cada situação criada.
    </p>
    <Figure
    ...
    <p>
      ...
      Dentro de cada sala também podem aparecer itens e monstros, existindo limites e parâmetros
      para estas gerações consoante à dificuldade do nível.
    </p>
    <p>
      O uso da geração procedural garante que os jogadores enfrentem uma experiência única, de forma
      que o layout da masmorra, o local dos itens e o tipo de inimigos encontrados são diferentes a
  ...
  <section>
    ...
    <p>
      Desde seu início como um pequeno jogo independente, o Minecraft já conquistou uma comunidade
      massiva e hoje é o jogo mais vendido de todos os tempos, tornando-se um evento global. No
      cerne dessa revolução virtual está não apenas a simplicidade dos blocos, mas uma complexidade
      oculta que alimenta a essência do jogo: o design procedural de mundos. A magia do Minecraft
      não reside apenas nos ambientes pixelados e na liberdade criativa ilimitada; ela é intrínseca
      à capacidade do jogo de gerar, de forma procedural, mundos vastos e interconectados.
    </p>
    <Figure
      src="minecraft.jpg"
    ...
    <p>
      ...
      próxima. Nessas camadas são utilizadas diversas técnicas, como por exemplo a criação de mapas
      de ruído (noise maps).
    </p>
    <p>
      Um noise map é uma imagem em que cada ponto (pixel) representa um valor limitado, geralmente,
      entre 0 e 1. Esta imagem depois pode ser utilizada para determinar uma propriedade de uma dada
      coordenada do mapa. Para determinar quais porções do mundo serão preenchidas com oceanos ou
      pedaços de terra firme, por exemplo, o Minecraft cria um noise map com a proporção de 1 para
      9, ou seja, para cada coordenada do mapa, é escolhido um número aleatório entre 1 e 10 e, se
      calhar 1, o local é um ponto de terra firme, senão, é oceano.
    </p>
    <p>
      Depois de determinados os biomas, o algoritmo escolhe um pixel e performa uma espécie de zoom.
      Cada pixel no noise map gerado anteriormente corresponde agora a um quadrado de 4096 por 4096
      blocos em que serão adicionados detalhes adicionais à região. Esta estratégia de ampliar o
      mapa anterior ainda é atrelada a uma outra forma de aleatorização. Ao fazer o “zoom”, o
      algoritmo muda propositalmente alguns dos pixels, resultando em ilhas menos quadradas e
      “imperfeições” as quais deixam o mundo mais natural e menos previsível.
    </p>
    <p>
      Depois de determinar quais áreas serão oceanos ou terra-firme, o algoritmo passa a calcular os
      biomas em cada região. Para isso utiliza-se de diversas camadas e novos noise maps para
      determinar quais as temperaturas em cada região, as quais podem ser: quente, temperado, frio
      ou congelante. Além de determinar uma temperatura, o gerador depois possui algumas camadas
      para garantir a suavidade dos biomas, isto é, que não haja um deserto escaldante do lado de
      uma tundra, por exemplo.
    </p>
    <p>
    ...
    <p>
      ...
      isso ele utiliza um noise map invertido: para cada ponto calcula um valor x e então remove x
      blocos a partir do topo do mapa (altura 320). O método para gerar o noise map é o Fractal
      Noise, o qual possui várias camadas em que cada camada tem um nível de contribuição diferente.
      Este método consegue gerar um efeito de continuidade no mapa à medida que adiciona pequenos
      detalhes, sem deformar de forma desproporcional a topografia do mapa.
    </p>
    <p>
